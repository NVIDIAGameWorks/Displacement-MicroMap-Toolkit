...
public:
  struct PrimitiveInstance
  {
    ...
    int           primMeshRef{0};  // reference to the array of primMesh
    int           material{0};     // unique primitive material
    std::string   name;
  };
  ...
  // Getters
  const std::vector<const ToolMesh*>& meshes() const { return m_constMeshes; }  // special case for a const ToolScene
  const std::vector<std::unique_ptr<ToolMesh>>& meshes() { return m_meshes; }
  const std::vector<const ToolBary*>& barys() const { return m_constBarys; }  // special case for a const ToolScene
  const std::vector<std::unique_ptr<ToolBary>>& barys() { return m_barys; }
  const std::vector<const ToolImage*>& images() const { return m_constImages; }  // special case for a const ToolScene
...
private:
  ...
  // A duplicate array of m_meshes, returned by const getters. This makes it
  // easier to find code that is accidentally able to modify const ToolScene
  // data.
  std::vector<const ToolMesh*> m_constMeshes;
  ...
  // A duplicate array of m_barys, returned by const getters. This makes it
  // easier to find code that is accidentally able to modify const ToolScene
  // data.
  std::vector<const ToolBary*> m_constBarys;
  ...
  // A duplicate array of m_images, returned by const getters. This makes it
  // easier to find code that is accidentally able to modify const ToolScene
  // data.
  std::vector<const ToolImage*> m_constImages;
