...
void main()
{
  ...
  if(hitKind == gl_HitKindFrontFacingMicroTriangleNV || hitKind == gl_HitKindBackFacingMicroTriangleNV)
  {
    ...
    payload.color = temperature(wPos.y * 2.0F);

    // Add wireframe
    if(pc.numBaseTriangles > 0)
    {
      const vec3  bary       = fract(vec3(1.0F - attribs.x - attribs.y, attribs.x, attribs.y) * pc.numBaseTriangles);
      float       minBary    = min(bary.x, min(bary.y, bary.z));
      const float thichness  = 0.002F * pc.numBaseTriangles;
      const float smoothness = 0.002F;
      const vec3  wire_color = vec3(0.3F, 0.3F, 0.3F);
      minBary                = smoothstep(thichness, thichness + smoothness, minBary);
