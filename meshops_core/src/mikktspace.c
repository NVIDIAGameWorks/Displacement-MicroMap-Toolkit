...
tbool genTangSpace(const SMikkTSpaceContext* pContext, const float fAngularThreshold)
{
  ...
  psTspace = (STSpace*)malloc(sizeof(STSpace) * (size_t)iNrTSPaces);
  if(psTspace == NULL)
  {
    free(piTriListIn);
    free(pTriInfos);
  ...
  memset(psTspace, 0, sizeof(STSpace) * (size_t)iNrTSPaces);
...
{
  const float fIndex = (float)g_iCells * ((fVal - fMin) / (fMax - fMin));
...
static void GenerateSharedVerticesIndexList(int piTriList_in_and_out[], const SMikkTSpaceContext* pContext, const int iNrTrianglesIn)
{
  ...
  // make allocations
  piHashTable   = (int*)malloc(sizeof(int) * (size_t)iNrTrianglesIn * 3);
  piHashCount   = (int*)malloc(sizeof(int) * (size_t)g_iCells);
  piHashOffsets = (int*)malloc(sizeof(int) * (size_t)g_iCells);
  piHashCount2  = (int*)malloc(sizeof(int) * (size_t)g_iCells);
  ...
  memset(piHashCount, 0, sizeof(int) * (size_t)g_iCells);
  memset(piHashCount2, 0, sizeof(int) * (size_t)g_iCells);
  ...
  pTmpVert = (STmpVert*)malloc(sizeof(STmpVert) * (size_t)iMaxCount);
...
static void InitTriInfo(STriInfo pTriInfos[], const int piTriListIn[], const SMikkTSpaceContext* pContext, const int iNrTrianglesIn)
{
  ...
  // match up edge pairs
  {
    SEdge* pEdges = (SEdge*)malloc(sizeof(SEdge) * (size_t)iNrTrianglesIn * 3);
    if(pEdges == NULL)
                             ...
                             const SMikkTSpaceContext* pContext)
{
  ...
  pSubGroupTspace = (STSpace*)malloc(sizeof(STSpace) * (size_t)iMaxNrFaces);
  pUniSubGroups   = (SSubGroup*)malloc(sizeof(SSubGroup) * (size_t)iMaxNrFaces);
  pTmpMembers     = (int*)malloc(sizeof(int) * (size_t)iMaxNrFaces);
  ...
  for(g = 0; g < iNrActiveGroups; g++)
  {
    ...
    for(i = 0; i < pGroup->iNrFaces; i++)  // triangles
    {
      ...
      {
        // insert new subgroup
        int* pIndices = (int*)malloc(sizeof(int) * (size_t)iMembers);
        if(pIndices == NULL)
        {
          // clean up and return false
          int s = 0;
        ...
        memcpy(pIndices, tmp_group.pTriMembers, (size_t)iMembers * sizeof(int));
